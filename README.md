...
- 在线演示（EdgeOne Production）：[打开 Personal Agent](https://personal-agent-cn.edgeone.cool/agent)
- 备用 Preview（Vercel）：[打开 Vercel Preview](https://personal-agent-2338176950-2036-jbx1.vercel.app/agent)
...
- 2026-08-11 已完成 Phase 4 公开 Smoke Test：Preview 无需登录；Voice Mode 在无麦克风权限时显示中文提示并保留文本输入；Mock Scenario 01 确认后 5 个 Tool 均为 `SUCCESS` 且 `Case：PASS`；真实 Agnes 首轮超时未执行 Tool，手动重试后生成 Structured Decision，确认执行后状态验证通过且 `Case：PASS`；390px 视口无横向溢出，Inspector 标签保持 3×2 排列。
- 2026-08-11 已新增 EdgeOne Production 镜像：`/agent` 可匿名访问，线上 `/api/agent/decision` 真实 Agnes Smoke Test 返回 `200` 和 Structured Decision。`AGNES_API_KEY` 仅保存为 EdgeOne Production 与 Vercel Preview 的敏感环境变量，不在 Git、README 或客户端代码中出现。
